...
class SaleTargetLine(models.Model):
    ...
    daily_target_qty = fields.Float(string='Daily Target Qty', compute='_compute_daily_target_qty', store=True)
        ...
        for line in self:
            ...
            if line.employee_id and line.product_id and line.target_id.date_from and line.target_id.date_to:
                domain = [
                    ...
                    ('order_id.date_order', '<=', line.target_id.date_to),
    ...
    @api.depends('target_qty', 'target_id.date_from', 'target_id.date_to')
    def _compute_daily_target_qty(self):
        for line in self:
            if line.target_id.date_from and line.target_id.date_to and line.target_qty:
                days = (line.target_id.date_to - line.target_id.date_from).days + 1
                if days > 0:
                    line.daily_target_qty = line.target_qty / days
                else:
