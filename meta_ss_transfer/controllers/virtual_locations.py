...
class MetaSSVirtualLocationController(http.Controller):
    ...
    @http.route(f"{API_PREFIX}/virtual-locations", type="json", auth="public", methods=["POST"])
    def get_virtual_locations(self, **payload):
        ...
        """
        try:
            domain = build_virtual_location_domain(request.env, payload)
    ...
    @http.route(f"{API_PREFIX}/virtual-locations/create", type="json", auth="public", methods=["POST"])
    def create_virtual_location(self, **payload):
        ...
        """
        try:
            vals = prepare_virtual_location_vals(request.env, payload)
            location = request.env["stock.location"].sudo().create(vals)
            ...
            distributor = request.env["res.partner"].sudo().browse(
                vals["ss_distributor_id"]
            )
            van_name = (payload.get("name") or "").strip()
            scrap_loc = create_van_scrap_sibling(
                request.env, location, distributor.name, van_name
            )
    ...
    @http.route(f"{API_PREFIX}/virtual-locations/<int:location_id>", type="json", auth="public", methods=["POST"])
    def get_virtual_location_detail(self, location_id, **payload):
        """Virtual location detail."""
        try:
            location = request.env["stock.location"].sudo().browse(location_id).exists()
